...
int mystoi(string s) {
    // getting unexpected out of range values from Z3 counterexample,
    // so using stol, which returns long int 
    if (check_hex(s))
        return stol(s, 0, 16);
    else
        return stol(s, 0, 10);
...
exp_node* copy_exp_tree(exp_node* root) {
    ...
    if (root == NULL)
        return NULL;  
    new_exp = new exp_node(root->type,
                           root->val,
...
void analyze_if(see_state* state) {
    ...
    state->path_const = update_path_const(state, state->path_const, false, root->exp_tree);
